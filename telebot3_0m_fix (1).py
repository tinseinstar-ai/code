...
def should_show_code(total_minutes):
    ...
    try:
        mins = int(total_minutes)
        # 0m ဆိုရင်လည်း ပြမယ်၊ 1440m (24h) ထက်ကျော်ရင်လည်း ပြမယ်။
        # User က 0m ကို သေချာပေါက် ပြစေချင်တာမို့ ဒီ logic က မှန်ပါတယ်။
                ...
                async with fresh_session.get(url, headers=headers) as req:
                    if req.status == 200:
                        ...
                        if respond.get('success'):
                            ...
                            # Try to get totalMinutes, fallback to remainingMinutes if needed
...
async def perform_check(session_url, code, chat_id, scan_id, message=None):
    ...
    for _attempt in range(3):
        ...
        token_match = re.search(r'token=([a-zA-Z0-9]{32})', response)
        if token_match:
            active_id = token_match.group(1)
        elif resp_json.get("success"):
            result = resp_json.get("result", {})
            if isinstance(result, str):
                t_match = re.search(r'token=([a-zA-Z0-9]{32})', result)
                if t_match: active_id = t_match.group(1)
            elif isinstance(result, dict):
                active_id = result.get("token") or result.get("sessionId") or session_id
        
        if 'logonUrl' in response or resp_json.get("success"):
            await handle_success(chat_id, code, active_id, message)
        elif 'STA' in response:
            await handle_limited(chat_id, code, active_id, message)
        break

async def handle_success(chat_id, code, active_id, message):
    expire_info, raw_minutes = await Code_Expires_Date(active_id)
...
async def handle_limited(chat_id, code, active_id, message):
    expire_info, raw_minutes = await Code_Expires_Date(active_id)
